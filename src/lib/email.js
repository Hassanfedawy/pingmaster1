...
// Debug function for production logging
const debugEmail = async (message, data = {}) => {
  console.log(`[Email Service] ${message}`, JSON.stringify(data, null, 2));
};
...
transporter.verify(function (error, success) {
  if (error) {
    debugEmail('SMTP Connection Error:', { error: error.message });
  } else {
    debugEmail('SMTP Connection Success', { success });
  }
});
...
export async function sendIncidentEmail(userEmail, incident) {
  try {
    debugEmail('Preparing to send incident email', { userEmail, incident });

    const { url, status, error, responseTime, timestamp } = incident;
    
    const emoji = getStatusEmoji(status);
    const statusColor = getStatusColor(status);
    const formattedTime = format(new Date(timestamp), 'PPpp');
    const timeAgo = formatDistanceToNow(new Date(timestamp), { addSuffix: true });
    
    const subject = `${emoji} Alert: ${url} is ${status.toUpperCase()}`;
    
    // Log email configuration (without sensitive data)
    debugEmail('Email configuration', {
      host: process.env.SMTP_HOST,
    ...
    debugEmail('Sending email with options', {
      to: mailOptions.to,
    ...
    debugEmail('Email sent successfully', {
    ...
    debugEmail('Failed to send incident email', {
      error: error.message,
      stack: error.stack,
      timestamp: new Date().toISOString()
    });
